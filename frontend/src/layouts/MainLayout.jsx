...
export function MainLayout({ navbar, title, description, image, noIndex = false, seoDisabled = false, children }) {
  const { pathname, search } = useLocation()
  const isAdminRoute = pathname.startsWith('/admin')
  const [cartToastMessage, setCartToastMessage] = useState('')

  const shouldRenderSeo = !seoDisabled || isAdminRoute || Boolean(title || description || image || noIndex)
